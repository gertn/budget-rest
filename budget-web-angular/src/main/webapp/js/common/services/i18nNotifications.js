...
angular.module('services.i18nNotifications').factory('i18nNotifications', ['localizedMessages', 'notifications', '$rootScope', 
                                                                           function (localizedMessages, notifications, $rootScope) {

	var resourceBundle = {};
	
	function resourceBundleLoaded(event, bundle){
		resourceBundle = bundle;
	};
	
	$rootScope.$on('resourceBundleLoaded', resourceBundleLoaded);
	
  var prepareNotification = function(msgKey, type, interpolateParams, otherProperties) {
     return angular.extend({
       message: localizedMessages.get(msgKey, interpolateParams, resourceBundle),
       type: type
     }, otherProperties);
  };
